        ...
        if (version_compare(strtolower($mysql_ver), "5.5.0") >= 0){
            foreach ($lines as $line){
                for($i = 0; $i< count($line); $i++){
                    if ($line[$i]->mType == 'EASYLEX_SQL_LETTER' && strcasecmp($line[$i]->mValue, "TYPE") === 0){
                        if (isset($line[$i+1]) && $line[$i+1]->mType == 'EASYLEX_SQL_MARK' && $line[$i+1]->mValue == "="){
                            $line[$i]->mValue = "ENGINE";
                        }
                    }
                }
            }
